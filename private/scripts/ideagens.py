...
class Db_Manager:
  ...
  def __init__(self, db_params=mongohq.ideagenstest):
...
if __name__ == '__main__':
    ...
    for prompt in prompts:
      users = db.get_users_in_prompt(prompt)
      for user in users:
        db.get_ideas_for_user(user, prompt)
        login_time = db.get_login_times([user,])
